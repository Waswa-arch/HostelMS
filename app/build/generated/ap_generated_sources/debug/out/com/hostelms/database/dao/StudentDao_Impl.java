...
public final class StudentDao_Impl implements StudentDao {
  ...
  public StudentDao_Impl(@NonNull final RoomDatabase __db) {
    ...
    this.__insertionAdapterOfStudent = new EntityInsertionAdapter<Student>(__db) {
      ...
      protected String createQuery() {
        return "INSERT OR ABORT INTO `students` (`id`,`name`,`gender`,`course`,`regNumber`,`email`,`password`,`phone`,`role`,`profilePhotoPath`,`age`,`roomId`,`bedNumber`,`admissionDate`,`amountOwed`) VALUES (nullif(?, 0),?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
      }
    ...
    this.__updateAdapterOfStudent = new EntityDeletionOrUpdateAdapter<Student>(__db) {
      ...
      @NonNull
      protected String createQuery() {
        return "UPDATE OR ABORT `students` SET `id` = ?,`name` = ?,`gender` = ?,`course` = ?,`regNumber` = ?,`email` = ?,`password` = ?,`phone` = ?,`role` = ?,`profilePhotoPath` = ?,`age` = ?,`roomId` = ?,`bedNumber` = ?,`admissionDate` = ?,`amountOwed` = ? WHERE `id` = ?";
      }
    ...
    this.__preparedStmtOfAssignRoom = new SharedSQLiteStatement(__db) {
      ...
      public String createQuery() {
        final String _query = "UPDATE students SET roomId = ?, bedNumber = ? WHERE id = ?";
        return _query;
      }
    };
  }
  ...
  @Override
  public List<Student> getStudentsByRoom(final int roomId) {
    final String _sql = "SELECT * FROM students WHERE roomId = ?";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 1);
    int _argIndex = 1;
